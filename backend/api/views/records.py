...
class RecordViewSet(viewsets.ModelViewSet):
    ...
    def history(self, request, pk=None, pipeline_pk=None):
        ...
        for log in audit_logs:
            ...
            elif log.action in ['created', 'deleted']:
                activity_type = 'system'
            elif log.action in ['external_edit', 'external_access', 'shared_record', 'share_revoked', 'share_deleted']:
                activity_type = 'system'  # Use system type for sharing activities
            else:
    ...
    def _format_audit_changes(self, changes, action):
        ...
        elif action in ['external_edit', 'external_access', 'shared_record', 'share_revoked', 'share_deleted']:
            # Handle sharing-related activities with rich formatting
            # Use primary_message if available, otherwise fall back to formatted secondary info
            if 'primary_message' in changes:
                primary = changes['primary_message']
                secondary = changes.get('secondary_message', '')
                if secondary:
                    return f"{primary}\n{secondary}"
                return primary
            
            # Fallback for sharing activities without rich messages
            if action == 'external_edit':
                accessor_name = changes.get('accessor_name', 'External user')
                return f"{accessor_name} edited this record"
            elif action == 'external_access':
                accessor_name = changes.get('accessor_name', 'External user')
                return f"{accessor_name} accessed this record"
            elif action == 'shared_record':
                intended_email = changes.get('intended_recipient_email', 'someone')
                return f"Record shared with {intended_email}"
            else:
                return f"Sharing activity: {action}"
