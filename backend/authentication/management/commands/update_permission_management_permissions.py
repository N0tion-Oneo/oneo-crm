"""
Management command to update existing user types with permission management permissions.

This command adds the new permission management actions (grant, revoke, assign, manage_roles) 
to existing user types based on their role level.
"""
...
class Command(BaseCommand):
    help = 'Update existing user types with permission management permissions'
    ...
    def update_tenant_permissions(self, dry_run=False):
        ...
        for user_type in user_types:
            ...
            # Check if permissions category needs updating
            current_permissions = user_type.base_permissions or {}
            needs_update = False
            
            # Check for missing or outdated permissions category
            current_permission_actions = current_permissions.get('permissions', [])
            expected_permission_actions = expected_permissions.get('permissions', [])
            
            if set(current_permission_actions) != set(expected_permission_actions):
                needs_update = True
                if not dry_run:
                    current_permissions['permissions'] = expected_permission_actions
                self.stdout.write(
                    f'  + Updating permissions actions for {user_type.name}: {expected_permission_actions}'
                )
