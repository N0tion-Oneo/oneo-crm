...
from .sync.config import get_sync_options
    ...
    try:
        ...
        for connection in whatsapp_connections:
            try:
                ...
                # Use sync options with user_initiated flag
                enhanced_options = {
                    **sync_options,
                    'user_initiated': True
                }
                ...
                # Validate proper tenant context exists
                if not hasattr(db_connection, 'tenant'):
                ...
                # Ensure we have a real tenant, not FakeTenant
                tenant = db_connection.tenant
                tenant_schema = tenant.schema_name
                
                # Validate this is a real tenant with domains
                if not hasattr(tenant, 'domains'):
                    logger.error(f"❌ Invalid tenant context - FakeTenant detected")
                    raise ValueError("Invalid tenant context. User must be logged in with proper tenant subdomain.")
                
                # Log tenant details
                logger.info(f"🏢 Tenant schema for background sync: {tenant_schema}")
                logger.info(f"🔍 Tenant name: {tenant.name}")
                tenant_domain = tenant.domains.first().domain if tenant.domains.exists() else 'No domain'
...
def start_chat_specific_sync(request, chat_id):
    ...
    try:
        ...
        # Enhanced sync options with chat-specific settings
        enhanced_options = {
            **sync_options,
        ...
        if not hasattr(db_connection, 'tenant'):
            logger.error(f"❌ No tenant context found - sync must be called from tenant subdomain")
            raise ValueError("No tenant context found. Chat sync must be called from a tenant subdomain.")
