...
Moved from pipelines/validators.py for better organization
"""
...
def validate_record_data(field_definitions: List[Dict[str, Any]], record_data: Dict[str, Any], context: str = 'storage') -> Dict[str, Any]:
    ...
    for field_def in field_definitions:
        ...
        # 🔧 FIX: For new records, assign proper default values for missing fields
        # This prevents the frontend from seeing None values which make fields uneditable
        if value is None and context in ['business_rules']:  # New records use business_rules context
            default_value = _get_field_default_value(field_type, field_config)
            print(f"🔧 DEFAULT VALUE: {field_slug} ({field_type.value}) missing, using default: {default_value}")
            value = default_value
        ...
        if not result.is_valid:
            ...
            # Use the result's cleaned_value, but preserve our assigned default values
            # This prevents the validator from overriding defaults with None
            cleaned_value = result.cleaned_value
            
            # If validator returned None but we had assigned a default value, keep the default
            if cleaned_value is None and value is not None:
                cleaned_value = value
                print(f"🔧 PRESERVE DEFAULT: {field_slug} keeping default {repr(value)} instead of validator's None")
            
            cleaned_data[field_slug] = cleaned_value
...
def _get_field_default_value(field_type: FieldType, field_config: Dict[str, Any]):
    """
    Determine appropriate default value for a field type (for new record creation)
    
    This ensures that new records get proper default values instead of None,
    which prevents fields from becoming uneditable in the frontend.
    ...
    if 'default_value' in field_config:
        print(f"🔧 FIELD DEFAULT: Using configured default: {field_config['default_value']}")
        return field_config['default_value']
    
    # Return type-appropriate default values
    if field_type in [FieldType.TEXT, FieldType.TEXTAREA, FieldType.EMAIL, FieldType.PHONE, FieldType.URL, FieldType.ADDRESS]:
        default_value = ""  # Empty string for text fields - editable
    elif field_type == FieldType.NUMBER:
        default_value = None  # Numeric fields should be null rather than 0
    elif field_type == FieldType.BOOLEAN:
        default_value = False  # Boolean default
    elif field_type == FieldType.DATE:
        default_value = None  # Date fields should be null
    elif field_type in [FieldType.SELECT]:
        default_value = None  # No default selection - but field will be editable
    elif field_type in [FieldType.MULTISELECT, FieldType.TAGS]:
        default_value = []  # Empty array for multi-value fields - editable
        ...
        default_value = None  # Relation fields should be null initially - but editable
    else:
        default_value = None  # Safe default for unknown types
    
    print(f"🔧 FIELD DEFAULT: {field_type.value} -> default: {default_value}")
