...
class AIMessageGeneratorProcessor(AsyncNodeProcessor):
    ...
    async def process(self, node_config: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        max_length = node_data.get('max_length', 500)
        ...
        target_channel = node_data.get('target_channel') or context.get('channel', 'email')

        try:
            # Build the AI prompt
            system_prompt = self._build_system_prompt(tone, target_channel, max_length)
    ...
    def _build_system_prompt(self, tone: str, channel: str, max_length: int) -> str:
        ...
        return f"""You are an AI assistant generating messages for business communication.
...
Focus on the core message content."""
    ...
    async def validate_inputs(self, node_config: Dict[str, Any], context: Dict[str, Any]) -> bool:
        ...
        if not node_data.get('prompt_template'):
            return False

        # Validate configuration values
        max_length = node_data.get('max_length', 500)
        if max_length < 10 or max_length > 4000:
            return False
