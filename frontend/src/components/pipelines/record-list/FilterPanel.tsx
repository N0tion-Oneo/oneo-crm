...
import { X, Plus, BookmarkPlus, Bookmark, Edit, Filter as FilterIcon, Check, AlertCircle, Lock, Building } from 'lucide-react'
import { 
  Filter, 
  FilterGroup, 
  BooleanQuery, 
...
import { SaveFilterModal, SavedFiltersList, SavedFiltersModal } from '@/components/pipelines/saved-filters'
...
export interface FilterPanelProps {
  ...
  currentSavedFilter?: any // The currently applied saved filter (if any)
  isFilterModified?: boolean // Whether the current filter has been modified from saved state
  onClearSavedFilter?: () => void // Callback to clear the saved filter state
  className?: string
}
...
export function FilterPanel({
  ...
  const [modalMode, setModalMode] = useState<'update' | 'new'>('new')
  ...
  // Handler for update existing filter
  const handleUpdateFilter = () => {
    setModalMode('update')
    setShowSaveFilterModal(true)
  }

  // Handler for save new filter
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <div className="flex items-center space-x-2">
            ...
            {currentSavedFilter && (
              <button
                onClick={() => setShowSavedFiltersModal(true)}
                className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border transition-all duration-200 hover:shadow-sm ${
                  isFilterModified 
                    ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800 text-orange-700 dark:text-orange-300 hover:bg-orange-100 dark:hover:bg-orange-900/30 shadow-orange-200/50 dark:shadow-orange-900/20 shadow-sm'
                ...
                title="Click to view all saved filters"
              >
                <FilterIcon className="w-3 h-3 mr-1.5" />
                <span className="font-medium">{currentSavedFilter.name}</span>
                {isFilterModified ? (
                  <AlertCircle className="w-3 h-3 ml-1.5 text-orange-500" />
                ) : (
                  <Check className="w-3 h-3 ml-1.5 text-blue-500" />
                )}
                {/* Access level indicator */}
                <div className="ml-1.5 flex items-center">
                  {currentSavedFilter.access_level === 'private' ? (
          ...
          {currentSavedFilter && (
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              {isFilterModified 
                ? `Modified filter • Click to manage or create new filter`
                : `Active saved filter • Click to switch or create new filter`
              }
            </p>
          )}
        </div>
        ...
        <div className="flex items-center space-x-2">
          {/* Update Filter Button - Only show when a saved filter is loaded */}
          {currentSavedFilter && (
            <button
              onClick={handleUpdateFilter}
              disabled={!hasActiveFilters && visibleFields.length === 0}
              className="px-3 py-1 border border-green-300 dark:border-green-600 text-green-600 dark:text-green-400 rounded-md hover:bg-green-50 dark:hover:bg-green-900/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center text-sm"
              title={`Update "${currentSavedFilter.name}" with current settings`}
            >
          ...
          <button
            ...
            title={currentSavedFilter ? "Save current settings as a new filter" : "Save current filter and view settings"}
          >
            <BookmarkPlus className="w-4 h-4 mr-1" />
            {currentSavedFilter ? 'Save New' : 'Save'}
          </button>

          {/* Saved Filters Modal Button */}
          <button
            onClick={() => setShowSavedFiltersModal(true)}
            className="px-3 py-1 border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-md flex items-center text-sm"
          >
          ...
          <button
            onClick={() => {
              ...
              // Clear the saved filter context
        ...
        <SaveFilterModal
          ...
          currentSavedFilter={modalMode === 'update' ? currentSavedFilter : null}
          forceMode={modalMode}
        />

        {/* Saved Filters Modal */}
        <SavedFiltersModal
          isOpen={showSavedFiltersModal}
          onClose={() => setShowSavedFiltersModal(false)}
          pipeline={{
            id: pipeline.id,
            name: pipeline.name
          }}
